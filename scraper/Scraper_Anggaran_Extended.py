...
# <div class="toc"><ul class="toc-item"><li><span><a href="#*Bot-Scraper-Anggaran-Command-Center-Kantor-Pertanahan" data-toc-modified-id="*Bot-Scraper-Anggaran-Command-Center-Kantor-Pertanahan-1"><span class="toc-item-num">1&nbsp;&nbsp;</span>*Bot Scraper Anggaran Command Center Kantor Pertanahan</a></span></li><li><span><a href="#Bot-Init!" data-toc-modified-id="Bot-Init!-2"><span class="toc-item-num">2&nbsp;&nbsp;</span>Bot Init!</a></span></li><li><span><a href="#Variables" data-toc-modified-id="Variables-3"><span class="toc-item-num">3&nbsp;&nbsp;</span>Variables</a></span></li><li><span><a href="#Login-SSO" data-toc-modified-id="Login-SSO-4"><span class="toc-item-num">4&nbsp;&nbsp;</span>Login SSO</a></span><ul class="toc-item"><li><span><a href="#Daftar-Nominatif-Pencairan" data-toc-modified-id="Daftar-Nominatif-Pencairan-4.1"><span class="toc-item-num">4.1&nbsp;&nbsp;</span>Daftar Nominatif Pencairan</a></span><ul class="toc-item"><li><span><a href="#Pengesahan-dan-DI.310" data-toc-modified-id="Pengesahan-dan-DI.310-4.1.1"><span class="toc-item-num">4.1.1&nbsp;&nbsp;</span>Pengesahan dan DI.310</a></span></li><li><span><a href="#Penerbitan-Sertipikat" data-toc-modified-id="Penerbitan-Sertipikat-4.1.2"><span class="toc-item-num">4.1.2&nbsp;&nbsp;</span>Penerbitan Sertipikat</a></span></li></ul></li></ul></li></ul></div>
...
#send_mail('reza.yoga@gmail.com', 'Bot Scrap Anggaran Extended Init', '[%s] Hi!\n<i>Bot Scrap</i> anggaran extended init:' % (getCurrentDate(True)))
...
import json
print('\n=> Daftar Nominatif Pencairan Pengesahan dan DI.310\n')
'''=============================================Daftar Nominatif Pencairan Pengesahan dan DI.310'''
'''
    ...
    (function ($) {
        ...
        $("#frmUsulanNominatif").submit(function (e) {
            if ($(this).valid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/Yuridis/NominatifPencairan/UsulanPengesahandanDI310',
                    data: $(this).serialize(),
        ...
        $("#btnprosesusulan").click(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/Yuridis/NominatifPencairan/ProsesUsulanPengesahandanDI310',
                data: pdata,
                success: function (data, textStatus, XMLHttpRequest) {
                    if (data.Status) {
        ...
        moreRowsUrl = '/Yuridis/NominatifPencairan/UsulanPengesahandanDI310';
...
def getDataUsulan(wilayahid, headers, pageNum):
    if wilayahid != '':
        url = 'https://ptsl.atrbpn.go.id/Yuridis/NominatifPencairan/UsulanPengesahandanDI310'
        payload = {
...
def get_data():
    ...
    with connection.cursor() as cursor:
        # Create a new record
        sql = "TRUNCATE `tb_daftar_nominatif_pencairan_pengesahan_dan_di310`"
        cursor.execute(sql)
                ...
                for y in range(1000):
                    ...
                    if df.empty:
                        ...
                        for index, row in df.iterrows():
                            ...
                            with connection.cursor() as cursor:
                                # Create a new record
                                sql = "INSERT INTO `tb_daftar_nominatif_pencairan_pengesahan_dan_di310` (nomor_berkas, nama_desa, kode_desa, nib, luas, created_at) VALUES (%s, %s, %s, %s, %s, %s)"
...
print('\n=> Daftar Nominatif Pencairan Penerbitan Sertipikat\n')
'''=============================================Daftar Nominatif Pencairan Penerbitan Sertipikat'''
'''
<script type="text/javascript">
    (function (a) { a.createModal = function (b) { defaults = { title: "", message: "Your Message Goes Here!", closeButton: true, scrollable: false }; var b = a.extend({}, defaults, b); var c = (b.scrollable === true) ? 'style="max-height: 420px;overflow-y: auto;"' : ""; html = '<div class="modal fade" id="myModal">'; html += '<div class="modal-dialog">'; html += '<div class="modal-content">'; html += '<div class="modal-header">'; html += '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>'; if (b.title.length > 0) { html += '<h4 class="modal-title">' + b.title + "</h4>" } html += "</div>"; html += '<div class="modal-body" ' + c + ">"; html += b.message; html += "</div>"; html += '<div class="modal-footer">'; if (b.closeButton === true) { html += '<button type="button" class="btn btn-primary" data-dismiss="modal" id="btn-ok">OK</button>' } html += "</div>"; html += "</div>"; html += "</div>"; html += "</div>"; a("body").prepend(html); a("#myModal").modal().on("hidden.bs.modal", function () { a(this).remove() }) } })(jQuery);
    ...
    (function ($) {
        ...
        $("#frmUsulanNominatif").submit(function (e) {
            if ($(this).valid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/Yuridis/NominatifPencairan/UsulanPenerbitanSertifikat',
                    data: $(this).serialize(),
                    success: function (data, textStatus, XMLHttpRequest) {
                        if (data == 'noresults') {
                            $("#lstberkasnominatif").html('Data tidak ditemukan.');
        ...
        $("#btnprosesusulan").click(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/Yuridis/NominatifPencairan/ProsesUsulanPenerbitanSertifikat',
                data: pdata,
                success: function (data, textStatus, XMLHttpRequest) {
                    if (data.Status) {
                        $('#daftarusulan').html(data.ListBerkas);
        ...
        moreRowsUrl = '/Yuridis/NominatifPencairan/UsulanPenerbitanSertifikat';
        $(window).scroll(scrollHandler);
    }(jQuery));
</script>
'''

@retry((Exception), tries=30, delay=15, backoff=0)
def getDataUsulan(wilayahid, headers, pageNum):
    if wilayahid != '':
        url = 'https://ptsl.atrbpn.go.id/Yuridis/NominatifPencairan/UsulanPenerbitanSertifikat'
        payload = {
          'programselected': program_id_ptsl,
          'Wilayah': wilayahid,
          'TahunBerkas': hari_ini.year,
...
def get_data():
    ...
    with connection.cursor() as cursor:
        # Create a new record
        sql = "TRUNCATE `tb_daftar_nominatif_pencairan_penerbitan_sertipikat`"
        cursor.execute(sql)
        # connection is not autocommit by default. So you must commit to save
        # your changes.
        print(cursor.rowcount, " current records.")
                ...
                for y in range(1000):
                    ...
                    if df.empty:
                        ...
                        for index, row in df.iterrows():
                            ...
                            with connection.cursor() as cursor:
                                # Create a new record
                                sql = "INSERT INTO `tb_daftar_nominatif_pencairan_penerbitan_sertipikat` (nomor_berkas, nama_desa, kode_desa, nib, luas, created_at) VALUES (%s, %s, %s, %s, %s, %s)"
                                cursor.execute(sql, (index, str(row['Nama Desa']), str(row['Kode Desa']), str(row['NIB']), str(row['Luas']), now))
                                # connection is not autocommit by default. So you must commit to save
                                # your changes.
                                #print(cursor.rowcount, "record inserted.")
...
# send_mail("dimasardi1349@gmail.com", "Bot Scrap Anggaran Extended Done", '[%s] Hi!\n<i>Bot Scrap</i> anggaran extended task done in %f minutes:' % (getCurrentDate(True), float(elapsed_time/60)))
